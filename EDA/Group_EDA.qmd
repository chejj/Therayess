...
##### Country, Ethnicity, Diet, and BMI

While country also contains data for all samples in our study, ethnicity and diet are minimally provided in

```{r, echo=FALSE}
#| label: Countries, Diet, and BMI associations
...
geography_BMI_caption <- str_wrap(
  "(A)  
   (B) 
   (C) 
   (D) 
...
country1 + country2 + country3 + diet1 + diet2 + guide_area() + bmi1 + bmi2
  plot_layout(design = layout1, guides = "collect", widths = c(1,1,1,1,0.7)) +
  plot_annotation(
    title = "Figure 2: Country, Diet, and BMI Associations with CRC Progression Classifiers",
    tag_levels = "A",
    caption = geography_BMI_caption
  ) &
  theme(
        plot.title = element_text(hjust = 0, size = 14),
        plot.caption = element_text(hjust = 0, size = 10), 
