...
# --- Initialization --- 
RELOAD_CRC_progression_studies <- list()
...
for (i in seq_along(studies)) { #seq_along() in R generates an integer sequence from 1 to the length of a given vector or list
  ...
  RELOAD_CRC_progression_studies[[study]] <- list() 
  ...
  for (type in focus) {
    ...
    if (!dryrun_mode) { # runs if downloading
      ...
      RELOAD_CRC_progression_studies[[study]][[type]] <- res[[1]]
    } else {
...
RELOAD_CRC_progression_studies <- lapply(RELOAD_CRC_progression_studies, function(st) lapply(st, filter_samples))
# Add Age_Decade 
RELOAD_CRC_progression_studies <- lapply(RELOAD_CRC_progression_studies, function(st) lapply(st, add_age_decade)) 
# Add Disease_Class 
RELOAD_CRC_progression_studies <- lapply(RELOAD_CRC_progression_studies, function(st) lapply(st, add_disease_class))
...
types_present <- lapply(RELOAD_CRC_progression_studies, names)
...
dim_report <- do.call(rbind, lapply(names(RELOAD_CRC_progression_studies), function(study) {
  st <- RELOAD_CRC_progression_studies[[study]]
...
alignment_report <- do.call(rbind, lapply(names(RELOAD_CRC_progression_studies), function(study) {
  st <- RELOAD_CRC_progression_studies[[study]]
...
meta_report <- do.call(rbind, lapply(names(RELOAD_CRC_progression_studies), function(study) {
  x <- RELOAD_CRC_progression_studies[[study]][["relative_abundance"]]  # pick one type for metadata checks
  if (is.null(x)) {
...
criteria_report <- do.call(rbind, lapply(names(RELOAD_CRC_progression_studies), function(study) {
  x <- RELOAD_CRC_progression_studies[[study]][["relative_abundance"]]
...
zero_sample_report <- do.call(rbind, lapply(names(RELOAD_CRC_progression_studies), function(study) {
  x <- RELOAD_CRC_progression_studies[[study]][["pathway_abundance"]]
  if (is.null(x)) return(NULL)
